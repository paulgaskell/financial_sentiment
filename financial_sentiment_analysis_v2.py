...
from config import CORPRA, ROLLING_VAR_PARAMS, FILTERS
...
class VAR:
    ...
    @staticmethod
    def get(ts, ticker, corpus, filter):
        vb = VARBundle(ts, ticker, corpus, filter)
...
class PanelVAR:   
    """Create panel regression
        the idea behind the filtering is that there may be catestrophic errors 
        because we are counting bad messages
    ...
    def get(ts):
        ...
        for corpus, filter in params_iter():
            X, y, vb = VAR.get(ts, tickers[0], corpus, filter)
            X = X.T
            for i in range(1, len(tickers)):
                X_, y_, vb = VAR.get(ts, tickers[i], corpus, filter)
            ...
            res_summary.append('{} {}\n{}\n'.format(corpus, filter,
                                    res.summary().as_text()))
            filtered_res = sm.OLS(filtered_y, sm.tools.add_constant(
                                    filtered_X)).fit()
            filtered_res_summary.append('filtered {} {}\n{}\n'.format(
                                    corpus, filter,
                                    filtered_res.summary().as_text()))
            
            logger.info('{}_{}'.format(corpus, filter))
            logger.info(res.summary().as_text())
            logger.info(filtered_res.summary().as_text())
            
        with open('panel_var_results_v2.txt', 'w') as out:
            out.write('\n'.join(res_summary))
...
class RollingVAR:
    ...
    def get(ts):
        ...
        for i in RollingVAR._rv_iterator(ts):
            ...
            X, y, vb = VAR.get(ts, ticker, corpus, filter)
...
class PortfolioAnalysis:
    ...
    @staticmethod
    def get(ts):
        port_returns = { '{}_{}'.format(corpus, filter): 
                            PortfolioAnalysis._get_port(corpus, filter) 
                            for corpus, filter in params_iter() }
        
        with open('portfolio_analysis_results_v2.txt', 'w') as out:
            iter = PortfolioAnalysis._port_results_iter(port_returns)
            for r, name, holding_period in iter:
                out.write('{}\n'.format(','.join(list(map(str, 
                            [name, holding_period, np.mean(r), len(r)]
                            )))))
        
        with open('ff_regression_results_v2.txt', 'w') as out:
